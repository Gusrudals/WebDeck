...
- **정준(canonical) SVG**: 요소 내부에 SVG 1개 —
  - `viewBox="0 0 100 100"`, `preserveAspectRatio="none"`(상자 크기 추종), `vector-effect="non-scaling-stroke"`(리사이즈 시 굵기 왜곡 방지)
  - `stroke="currentColor"` — 선 색은 요소 extraStyle의 `color`로 제어. 삽입 기본 `color: #374151`
  - 굵기 고정 `stroke-width="2"` (조절 UI는 후속)
  - 화살표 머리는 marker가 아닌 **polygon을 SVG에 직접 포함**(marker id 충돌·문서 간 복사 문제 회피). 머리는 non-scaling이 아니므로 크기에 따라 변형될 수 있음 — 선 요소의 표준 프레임(가늘고 긴 상자)에서는 체감 미미, 수용
...
- 화살표 머리는 상자 비율에 따라 약간 변형될 수 있음(표준 얇은 프레임에서 체감 미미)
