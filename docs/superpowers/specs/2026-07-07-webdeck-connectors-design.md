...
- 두 점 p1→p2 드래그, frame = 두 점 bbox. **bbox 한 축이 1px 미만(거의 완전 수평/수직 드래그)이면 그 축을 8px로 하고 그 축 점들은 50%** — §5 편집 재정규화의 0 나눗셈 가드와 같은 임계(1px)로 통일한다. 1~8px의 얇은 축은 그대로 두어 사용자의 draw를 왜곡하지 않는다(거의 직선인 elbow는 line 선택이 맞으며 핸들·재그리기로 정정 가능):
